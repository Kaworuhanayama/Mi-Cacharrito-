...
public class controladorAutenticacion {
    ...
    public ResponseEntity<?> registrarUsuario(@RequestBody SolicitudRegistroUsuario solicitud) {
        if (solicitud == null || tieneCamposVacios(solicitud.identificacion(), solicitud.nombreCompleto(),
                solicitud.categoria(), solicitud.correoElectronico(), solicitud.numeroTelefono(),
                solicitud.password()) || solicitud.fechaExpedicionLicencia() == null || solicitud.vigencia() == null) {
            return respuestaError(HttpStatus.BAD_REQUEST, "Todos los datos del registro son obligatorios");
        }

        if (usuarioRepositorio.existsByIdentificacion(solicitud.identificacion())) {
            return respuestaError(HttpStatus.CONFLICT, "La identificacion ya esta registrada");
        }

        if (usuarioRepositorio.existsByCorreoElectronico(solicitud.correoElectronico())) {
            return respuestaError(HttpStatus.CONFLICT, "El correo electronico ya esta registrado");
        }
        ...
        return ResponseEntity.status(HttpStatus.CREATED).body(
                crearRespuesta(usuarioGuardado, "Usuario registrado correctamente"));
    }
    ...
    private ResponseEntity<?> autenticar(SolicitudLogin solicitud, String rolEsperado) {
        if (solicitud == null || tieneCamposVacios(solicitud.identificacion(), solicitud.password())) {
            return respuestaError(HttpStatus.BAD_REQUEST, "La identificacion y el password son obligatorios");
        }

        Usuario usuario = usuarioRepositorio.findByIdentificacion(solicitud.identificacion()).orElse(null);
        if (usuario == null || !usuario.getPassword().equals(solicitud.password())
        ...
        return ResponseEntity.ok(crearRespuesta(usuario, "Inicio de sesion exitoso"));
    }

    private RespuestaAutenticacion crearRespuesta(Usuario usuario, String mensaje) {
        return new RespuestaAutenticacion(mensaje, usuario.getIdUsuario(), usuario.getIdentificacion(),
